...
class PrisonersDilemma(BedrockChat):
    ...
    def play_round(self, total_actions_made : list[list[str]]) -> tuple[list[str], list[int]]:
        ...
        for idx, player in enumerate(self.players):
            other_idx = 1 - idx
            recommendation = "Please choose your action."
        ...
        for idx, player in [
            ...
            response_text, tokens = self._player_response(player, total_actions_made)
            ...
            # that response has already passed the _is_action_message() check
            # so we can safely parse the action
            actions_made[idx] = self._parse_action(response_text)
            player.append_context(
                PlayerRole.ASSISTANT,
                response_text,
            )
            player.append_context(
                PlayerRole.USER,
                "[hint] Action chosen. Waiting for the game to end.\n",
            )
        ...
        return actions_made, token_counts

    def _player_response(
        self,
        player : Player,
        total_actions_made : list[list[str]],
    ):
        if (player.k < 2):
            return self._player_response_aux(player, total_actions_made)
        else:
            total_tokens = 0
            responses = list()
            ...
            for i in range(player.k):
                ...
                response, tokens = self._player_response_aux(new_player, total_actions_made)
                total_tokens += tokens
                responses.append(response)
                players.append(new_player)
            
            # voting phase - most popular action wins, pick randomly a response with that action
            action_counts = Counter(self._parse_action(response) for response in responses)
            ...
            for idx, response in resp_aux:
                if self._parse_action(response) == max_action:
    ...
    def _player_response_aux(
        ...
        error_cnt = 0
        while True:
            response_obj = player.generate_response(total_actions_made)
    ...
    def _validate_response(self, msg : str) -> tuple[bool, str]:
        ...
        if self._is_action_message(msg):
            return self._validate_action(msg)
        else:
            return False, f"You may structure your response however you like, but it should contain an action message. Action messages begin with the tag [action] and not contain other tags, which is followed by your optional explanation in parentheses, and end with a valid action: '{self.a}' or '{self.b}'.\nFormat: [action] (Optional explanation here) Your action here"

    def _is_action_message(self, msg : str) -> bool:
        """
        ...
        -------
        bool
            whether the message is an action message
        """
        pattern = r"\[[^\]]+\]"
        matches = re.findall(pattern, msg)
        return len(matches) == 1 and matches[0] == "[action]"
    
    def _validate_action(self, msg : str) -> tuple[bool, str]:
        """
        ...
        pattern = rf'\[action\](?: \(([^)]+)\))? ({re.escape(self.a)}|{re.escape(self.b)}|)'
    ...
    def _calculate_points(self, actions_made : list[str]) -> tuple[int, int]:
        """
        ...
        actions_made : list[str]
            list of actions made by the players
        ...
        """
        if None in actions_made:
            return (0, 0)
        
        action1, action2 = actions_made
        if action1 == self.a and action2 == self.a:
            return (self.aa, self.aa)
        if action1 == self.b and action2 == self.b:
            return (self.bb, self.bb)
        if action1 == self.a and action2 == self.b:
            return (self.ab, self.ba)
        if action1 == self.b and action2 == self.a:
    ...
    def play(self, rounds : int) -> dict:
        ...
        """
        # log game start
        action_mapping_str = json.dumps({ "a": self.a, "b": self.b }, indent=2)
        ...
        self.save_log(action_mapping_str + "\n" + payoff_matrix_str + "\n")
        self.save_log(f"{rounds} rounds.\n")
        for player in self.players:
            player.save_log(action_mapping_str + "\n" + payoff_matrix_str + "\n")
            player.save_log(f"{rounds} rounds.\n")
        

        total_tokens = list()
        total_actions_made = list()
        total_points = list()
        for r in range(rounds):
            round_actions_made, token_counts = self.play_round(total_actions_made)
            total_tokens.append(token_counts)
            round_points = self._calculate_points(round_actions_made)
            ...
            self.save_log(f"Round {r} results: {round_actions_made}, {round_points}\n")

            if None not in round_actions_made:
                if winner_idx == -1:
                    for player in self.players:
                        player.append_context(
                            PlayerRole.USER,
                            f"[hint] The round ended. Both players choose the same action: {round_actions_made[winner_idx]}.\nYou scored {round_points[0]} points.\n",
                        )

                else:
                    self.players[winner_idx].append_context(
                        PlayerRole.USER,
                        f"[hint] The round ended. You choose the action: {round_actions_made[winner_idx]}. Your opponent choose the action: {round_actions_made[1-winner_idx]}.\nYou scored {round_points[winner_idx]} points.\n",
                    )
                    self.players[1 - winner_idx].append_context(
                        PlayerRole.USER,
                        f"[hint] The round ended. You choose the action: {round_actions_made[1-winner_idx]}. Your opponent choose the action: {round_actions_made[winner_idx]}.\nYou scored {round_points[1-winner_idx]} points.\n",
                    )
            else:
                for player in self.players:
                    player.append_context(
            ...
            total_actions_made.append(round_actions_made)
            ...
            info = self._generate_info(total_tokens, total_actions_made, total_points)
    ...
    def _generate_info(
        ...
        total_actions_made : list[list[str]]
            list of actions made by the players for each round
        total_points : list[list[int]]
        ...
        info["valid_outcomes"] = [None not in actions_made for actions_made in total_actions_made]
        for i, player in enumerate(self.players):
            ...
            info[f"player_{i}_moves"] = [actions[i] for actions in total_actions_made]
